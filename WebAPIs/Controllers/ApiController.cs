...
namespace WebAPIs.Controllers
{
    ...
    public class ApiController : Controller
    {
                ...
                else
                {
                    ...
                    foreach (Contacts item in conts)
                    {
                        if (item != null)
                        {
                            ...
                            List<Messages> mess =
                            db.Messages.Where(o => o.FromUserName == username || o.ToUserName == item.FriendUserName
                            || o.FromUserName == item.FriendUserName || o.ToUserName == item.UserName).ToList();
                            object messages;
                            foreach (Messages items in mess)
                            {
                                if (items != null)
                                {
                                    bool fromMe = false;
                                    if (items.FromUserName == username)
                                        fromMe = true;
                                    object tempmess = new
                                    {
                                        id = items.Id,
        ...
        [Route("/api/register/")]
        [HttpPost]
        public async Task <ActionResult> Register(IFormFile avatar, [FromForm] string username, [FromForm] string name, [FromForm] string password)
        {
            try
            {
                ...
                {
                    string p = null;
                        var filePath = "wwwroot/images/" + "00" + avatar.FileName;
                        if (avatar != null)
                        {
    ...
    || o.FromUserName == item.FriendUserName || o.ToUserName == item.UserName).ToList();
        ...
        [Route("/api/contacts")]
        [HttpGet]
        public ActionResult NewContact1([FromForm] string uname)
        {
            try
            {
                Contacts con = db.Contacts.FirstOrDefault(o => o.FriendUserName == uname );
                if (con != null)
                {
                    return Json(new { code = HttpStatusCode.Conflict, error = "already a friend" });
                ...
                else
                {
                    ...
                    if (u.Username != uname )
                        return Json(new { code = HttpStatusCode.NotFound, error = "doesn't exist" });
                    var qq = from v in db.Contacts
                             where v.FriendUserName == u.Username
                             select v;
